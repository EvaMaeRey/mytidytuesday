...
ggplot2 is quite logical and accessible, but may present some challenges for young learners given more limited vocabulary and having less practice tolerating frustration.  
...
1. `ggkids()` is an alias for `ggplot()`.  Either can be used to initialized the plot and set global data. `ggkids` might be preferred if you want to more strongly signal 'we made this just for you, kiddos!'.
2. `use` is an alias for `aes`, since 'aesthetic' is unlikely to be already in the vocabulary of young learners.  We think that 'use color' (to as the aesthetic channel to communicate about 'var1') or 'use size' might be a natural way to talk about aesthetic mapping with children.
