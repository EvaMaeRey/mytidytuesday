...
> What worked for me was to remove the conditional `data$group[1] == -1` from `StatCircle` and just replacing any existing `group` value with row indices.

> This might be inadvisable, since in general it's bad to overwrite the user's `group` assignment; but i think it would be a rare situation for something that behaves like `StatIdentity` with `GeomPoint` to fail because of a bad `group` assignment. My understanding is that `group` only really matters to the statistical transformation, e.g. grouping points before computing an ellipse or convex hull; so here it's safe to "appropriate" it for another purpose since there'll be no conflict.

> An alternative would (i think) be for the stat to define a new, custom `group`-like column (a computed variable) that would be recognized by a companion geom. This violates the "pairability principle" one might call it—that new stats and geoms should be designed to pair with existing geoms and stats—but that's a principle i personally like that i think most extenders don't follow closely. (We've talked about it in discussions.) But it might be more appropriate because it acknowledges that the structure of the graphical element—differently-sized circles around points that may also be grouped by color and opacity—is different from the simpler structure of a collection of paths that would be passed to `GeomPolygon`.
...
StatCircle <- 
  ggproto("StatCircle", 
          ...
          compute_group = function(data, scales, n = 5){
            create_circle(data, n = n)},
          required_aes = c("x0", "y0", "r")
                      )
...
![using facetting](faceting.png)

hat worked for me was to remove the conditional `data$group[1] == -1` from `StatCircle` and just replacing any existing `group` value with row indices.
