...
> **'Every dot here represents one day of the S&P 500 Index’s return.'** *Viz exploration move:* `aes(id = date) + 
  geom_circlepack()`
...
> **'Here are those days, colored by the return on that day.'** *Viz exploration move:*  `aes(fill = return) `
...
> **'Even from a glance, you can see the volatility ...'** *Reflection based on viz* 


> **'... of the pandemic era, as well as the bear market of 2022.' ** *Viz exploration move:* `facet_wrap(~ year)`
...
>  **'Although there have been worse days,'** *Reflection/viz move:* `facet_null()` to return to 10-year lumped together.
...
> **'...what was remarkable about yesterday was how the good mood soured. '** *Forshadowing:*


> **'Here are all of the days again, this time arranged by how much the S&P 500 opened up in its very first trade of the day.'**  *Viz exploration move:* `aes(x = open_pct) + geom_beeswarm()`
...
> **'... but now let’s only show the days when the S&P 500 Index opened up more than 1%.'** *Viz exploration move:*  `data_filter(open_pct > 1)` 
...
> **'Such a good mood at 9:30 a.m. ET is pretty rare — only on 61 occasions out of 2,739 days did the market open up >1%.'** *Reflection* 
...
> **'Both of those anomalies happened in 2025 (`facet_grid(year~.)` helps us look year-wise), and one of them was yesterday, when stocks opened up nearly 1.44%, only to slide nearly the entire session, closing down 1.56%.'** *Viz exploration move/reflection:*
