...
if we reorganize, we'll use positioning, we can do oneliners.

```{r}
stat_r <- function(compute_group_fun, geom, ...){
...
```{r, warning=F}
geom_xmean <- function(...){stat_r(function(df) df |> summarize(xintercept = mean(x)), "vline", ...)}
