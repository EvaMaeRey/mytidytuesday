...
```{r, warning=F}
library(tidyverse)
geom_xmean_line <- function(...){rasa_g(function(df) df |> summarize(xintercept = mean(x)), "vline", ...)}
...
geom_xmean <- function(...){rasa_g(function(df) df |> summarize(x = mean(x), y = I(.05)), "point", ...)}
...
geom_proportion_label <- function(...){rasa_g(function(df) df |> summarise(x = sum(x)/length(x), y = 0) |> mutate(label = round(x,3)), vjust = 0, "text", ...)}   # this should work for T/F too when rasa_p is in play
...
```{r}
geom_grouplabel_at_means <-  function(...){rasa_g(function(df) df |> group_by(label) |> summarize(x = mean(x, na.rm = T), y = mean(y, na.rm = T)), "label", ...)}
