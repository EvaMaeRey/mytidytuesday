...
geom_ymean_line <- function(...){
    ...
    stat_group(function(df) df |> summarize(yintercept = mean(y)), 
             "hline", 
             dropped_aes = c("x", "y"), ...)
...
geom_ymean_label <- function(round = 0, ...){
  ...
  stat_group(function(df) df |> summarise(y = mean(y), x = I(.5)), 
             geom = "label", 
...
geom_xmean_line <- function(...){
    ...
    stat_group(function(df) df |> summarize(xintercept = mean(x)), 
             "vline", 
             dropped_aes = c("x", "y"), ...)
...
geom_xmean_label <- function(round = 0, ...){
  ...
  stat_group(function(df) df |> summarise(x = mean(x), y = I(.5)), 
             geom = "label", 
...
geom_x_sd <- function(sd = 1, ...){
    ...
    stat_group(function(df) df |> summarize(
...
ggplot(births_df_pruned_detrend_1) +
  ...
  ggxmean:::stamp_normal_dist(sd = 361.3038, height = .0038, aes(fill = "Normal overlay", color = "Normal overlay"), alpha = .3)
