...
*I use function geom_x_mean() from [ggxmean](https://github.com/EvaMaeRey/ggxmean) to quickly show and annotate the mean value.*
...
For each day we plot the 95% confidence interval around the mean based on [t-tests](https://github.com/EvaMaeRey/ggxmean/blob/master/R/ttest.R); these are visualized as a red horizontal segment at the base of the histogram.  The intervals are calculated independently by panel. 
...
To look at the within-year periodicity, we use a normalized date variable, 'date_as_if_2020'.  We see summer and fall surges in number of births.
...
## Narrowing to calendar date peers; modeling with year and day-of-week indicator and date in year (continuous) variables
...
last_plot() + 
  ...
  aes(color = year) + 
  geom_smooth() + 
  labs(title = "Toward seasonality detrend via OLS")
```
...
model_detrend_2 <- loess(num_births_detrend ~ as.numeric(date_as_if_2020), 
                 data = births_df_pruned_detrend_1, span = .75)
