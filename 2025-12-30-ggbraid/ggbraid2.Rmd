...
df_interpolate <- function(data, n = 80) {
  
  data_sorted <- data |> arrange(x)
  
  x_seq <- seq(min(data_sorted$x), max(data_sorted$x), length.out = n)
  
  data_expanded <- tibble(x = x_seq) |>
    # Union the evenly spaced x values with the original x0 values,
    # keeping all unique x points from both sets
    bind_rows(data_sorted |> select(x)) |>
    distinct(x) |>
    arrange(x)
  ...
  data_expanded %>%
    mutate(
      y = inter_y$y,
      is_original = x %in% data_sorted$x
    )
  
}

data.frame(x = c(1, 2, 8), y = c(1,0,5)) |> 
  df_interpolate() |>
  ggplot() + 
  aes(x, y) + 
  geom_point()
...
```{r}
set_mist_gradient <- function(colour = "coral"){
  
  c(colour |> alpha(.2),   #<<
                 "transparent", 
                 "transparent") |> #<<
  grid::linearGradient(x1 = 0, 
                       x2 = 0,
                       y1 = 1, 
                       y2 = 0)  #<<
...
continuous_ribbon_df |> 
  ...
  geom_ribbon(fill = set_mist_gradient()) + 
  aes(group = group)
