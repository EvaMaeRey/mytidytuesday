...
`r flipbookr::chunk_reveal("StatXymedians", left_assign = "detect", widths = c(1,.2), break_type = 1)`


```{r StatXymedians, include = TRUE}
...
`r flipbookr::chunk_reveal("layerStatXymedians", left_assign = "detect", break_type = 1)`
...
last_plot() + 
  # Note, computation is within group so ID #1 is seen twice
  aes(color = dist > 60) 
```
