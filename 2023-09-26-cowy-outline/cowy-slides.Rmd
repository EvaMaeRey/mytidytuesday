...
`r chunk_reveal("scatter", title = "# We all have Hans Rosling capabilities", break_type = "user")`

```{r scatter, include = F}
library(tidyverse)
library(gapminder)
gapminder %>%  # data from package
  filter(year == 2002) %>% # BREAK0
  ggplot() + # BREAK1
  aes(y = lifeExp) + # BREAK2
  aes(x = gdpPercap) + # BREAK3
  scale_x_log10() + # BREAK4
  geom_point() +  # BREAK5
  aes(size = pop/1000000 # BREAK6
  aes(color = continent) # BREAK7
  aes(shape = continent) # BREAK8
  labs(x = "GDP per capita (US$)") + # BREAK9
  labs(alpha = "GDP per cap") # BREAK10
```
...
# Like geom_smooth does a bunch of smoothcompute in the background
...
# The problems for a 'new wave' quoting June Choe*... 
