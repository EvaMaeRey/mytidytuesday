...
subtitle: "Easy geom_*() recipes: introducing layer extension"
...
## What the next release means for the recipes
...
> ### "When I need to make sense of some data and [ggplot2] continues to be just the best thing ever." -- Dewey Dunningham
...
> # "I'd never know quite how to get things done with matplotlib.  But with ggplot2 I could understand it without even looking at documentation - Hassan Kibirige (paraphrase)
...
## And started to feel a a little bit of pain even w/ ggplot2. 
...
## Saw Thomas Lin Pederson's talk 'Extending your ability to extend ggplot2' January 2020  geom_circle()
...
## 3 examples, 3 exercise, 3 objectives:

--

- 1. How to prepare compute for group-wise computation in a layer

--

- 2. How to define a Stat (and test)

--

- 3. How to combine a Stat and Geom into a user-facing function
...
- step-by-step 
- pre step - connect it to what you know (get the job done with base ggplot2)
...
```{r, echo = T, fig.show="hold", out.extra="25%"}
library(ggplot2)
ggplot(airquality) +
  aes(x = Ozone) +
  geom_histogram() +
...
```{r, fig.show="hold", out.extra="25%", echo= T}
library(statexpress)

compute_means <- function(data, scales){ 
  data |> 
    summarise(xintercept = mean(x, na.rm = T)) 
  }

ggplot(airquality) +
  aes(x = Ozone) +
  geom_histogram() +
  qlayer(geom = GeomVline, stat = qstat(compute_means))
...
## So...  Now there are 4 recipes, where compute_panel is introduced in recipe 2 and used in 3 and 4. 
...
## Do you still feel motivated by compute_group Stat example?
...
geom_vline <- function(mapping = NULL, data = NULL, 
  ...
  layer(
    ...
    inherit.aes = missing(xintercept),
    params = rlang::list2(
      na.rm = na.rm,
      ...
    )
...
ggplot(airquality) +
  aes(x = Ozone) +
  geom_histogram() +
  geom_vline(stat = qstat(compute_means))
