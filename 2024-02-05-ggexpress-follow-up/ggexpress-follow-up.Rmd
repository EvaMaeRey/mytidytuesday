...
> But still, creating a geom from scratch is an involved process that doesn’t lend itself to simple transformations. -- Elio Campitelli  *Pero aún así, hacer un stat específico cada vez que uno quiere hacer un plot en particular es demasiado complicado.*

I worked with stat_rasa and it is good.  It seems very similar to the ggtemp goals.  I think ggtemp might be motivated by the idea that eventually you might want to move to full, standard extension syntax, and then maybe you'd be more positioned to do that? 

I heard about https://github.com/mjskay/ggbuilder from June Choe and think that is about even more flexibility?  Redefine compute internals on the fly?  I'm not sure. I also include some code on piping, but to be honest, practical implications are not clear to me. 😭

I tried to make some changes to ggtemp functions to make the internal code more compact... but I think (hope) my ggplot2::stat knowledge is failing me, and hope there is a solution.
...
## Compare with ggtemp formulation {.tabset}
...
ggbuilder and pipe note might have something to do with more dynamic, on-the-fly internal data manipulation and stat creation...
