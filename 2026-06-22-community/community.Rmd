...
### ggdims: ggplot2 but where ‘aggregation’ can be as easily handled across features (columns) as it is to aggregate across samples (rows).
...
> is gonna be really useful repeatedly and that's the red star. 
...
means that we are often just a grammar of graphics 'move' (or a couple) away from the
