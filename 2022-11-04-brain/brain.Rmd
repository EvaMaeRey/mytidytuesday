...
Also, it often requires preprocessing to work with data you'd like to represent.

When we ask the question 'where' about maps, the answer is often a named region (polygon), and not x and y; it is the geometry.  Is it possible to create an interface where you define 'where' with these named geometries?  In other words: `aes(country = my_countries_variable)` to be used with a geom/stat like geom_polygon_countries().
...
# Step 1: compute group function and test
