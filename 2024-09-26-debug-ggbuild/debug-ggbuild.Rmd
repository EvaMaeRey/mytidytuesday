...
# {
  ...
  scales <- plot$scales; #scales # step 1.
  ...
  layout <- ggplot2:::create_layout(plot$facet, plot$coordinates, plot$layout); #layout # step 7.
  ...
  scale_x <- function() scales$get_scales("x"); scale_x # step 12
  scale_y <- function() scales$get_scales("y"); scale_y # step 13
  layout$train_position(data, scale_x(), scale_y()); #layout # step 14
  ...
  plot$scales$add_missing(c("x", "y"), plot$plot_env) # step 19
  data <- ggplot2:::by_layer(function(l, d) l$compute_geom_1(d), layers,  data, "setting up geom"); data # step 20
  data <- ggplot2:::by_layer(function(l, d) l$compute_position(d, layout),  layers, data, "computing position") # step 21
  data <- .ignore_data(data); data # step 22 
  layout$reset_scales(); #layout # step 23
  layout$train_position(data, scale_x(), scale_y()); #layout # step 24
  layout$setup_panel_params(); #layout # step 25
  data <- layout$map_position(data); data # step 26
  layout$setup_panel_guides(plot$guides, plot$layers); #layout # step 27
  npscales <- scales$non_position_scales(); npscales # step 28
  if (npscales$n() > 0) {
  ...
  data <- ggplot2:::by_layer(function(l, d) l$finish_statistics(d), 
    layers, data, "finishing layer stat") # step 32
