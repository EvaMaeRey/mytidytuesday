...
New Stat functionality can help ggplot2 become more expressive and more of a joy to use.  However, guidance for creating new layers has typically involved a fair amount of 'scaffolding' code, which is hard to remember.  
...
## Step 0. Status Quo, no-extension build. 
...
oregon_population_pyramid_data %>% 
  filter(county == "Baker") %>%
  mutate(split = ifelse(gender == "Men", sep, -sep)) %>% 
  mutate(xmin = 0 + split) %>% 
  mutate(xmax = percent * ifelse(gender == "Men", 1, -1) + split) %>% 
