...
# Trying human anatomy with same routing (list to sf) fails... 🥲

```{r, error = T}
gganatogram::hgFemale_list |>  #human female instead of mouse
  bind_rows() |>
  # filter(x != 0, y != 0, y < -2) |>
  remove_missing() |>
  to_sf_routine() 
...
gganatogram::hgMale_list |>  #human female instead of mouse
  bind_rows() |>
  # filter(x != 0, y != 0, y < -2) |>
  remove_missing() |>
  to_sf_routine() 
...
```{r}
library(teethr)
teeth_ref_data <- dental_arcade_mapping |> as_tibble()
