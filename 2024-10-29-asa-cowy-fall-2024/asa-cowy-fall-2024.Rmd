...
Without geom_bumb(), drawing a bumpchart would be a non-starter in most settings (business, government, academic).

--

Layer extension can deliver high value. 
...
Delivering layer extension can be costly.
...
(These people may have staggeringly good extension ideas, and academic setting has potential to have ripple effect since educators have a lot of trainees...) :-)
...
no packaging - in-script extension
...
stamp_prop_label <- function(...){  
  qlayer(geom = qproto_update(GeomLabel, 
                              aes(fill = NA, label.size = NA, vjust = 0, color = "red")),
         stat = qstat_panel(compute_panel_prop_asserted_label), 
         ...)
  }
...
## *Long statistical graphical poems.*... are being written on the board all the time by stat's instructors.
...
### ggplot2 *code* sometimes allows us to write those poems down in concise, logical, consistent ways... 

--

The code is an expression of the actual poem.
...
data.frame(wt = c(compute_chars, stat_chars + layer_chars, compute_chars, qlayer_qstat_chars),
  ...
  labs(title = "Creating geom_support with new classic v. statexpress method")
```
...
```{r}
layers_proportion_test <- function(asserted = .5, ...){
...
# Part 5. Minimally package. Enjoy in classroom as instructor/student.
...
# What are these ggexpress functions?
