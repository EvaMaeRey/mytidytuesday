...
Everyday ggplot2 extension: New approaches to light-weight 'geom' extension
...
donor <- read_delim(isi_donor_url) %>%
  mutate(decision = fct_rev(Choice))
...
only 20 percent defines new compute of interest!?



```{r, echo = F, message = F}
knitrExtra::chunk_code_get("compute_panel_support") %>% nchar() %>% sum() -> compute;compute

knitrExtra::chunk_code_get("StatSupport") %>% nchar() %>% sum() +
knitrExtra::chunk_code_get("geom_support") %>% nchar() %>% sum() ->
  stat_and_layer; stat_and_layer

data.frame(wt = c(compute, stat_and_layer),
...
```{r}
knitrExtra::chunk_code_get("compute_panel_support") %>% str_remove_all(" ") %>% nchar() %>% sum() ->
compute;compute

knitrExtra::chunk_code_get("geom_support_express") %>% str_remove_all(" ") %>% nchar() %>% sum() ->
  geom_support_express

geom_support_express + compute

compute/(geom_support_express + compute)

data.frame(wt = c(compute, stat_and_layer, compute, geom_support_express),
           code_type = rep(c("compute code", "scaffolding code"), 2),
           strategy = c("classic", "classic", "statexpress", "statexpress")) |>
  ggplot() + 
  ...
  labs(title = "Creating geom_support with statexpress")
```
...
'That is a bunch of scaffolding code and I won't bore you with that'.
...
So I want to alert you to some headway on this problem, w/ caveats.
...
multi-step process, and verbose
...
```{r, eval = F}  
gapminder::gapminder %>% 
  filter(country %in% c("United States", "Germany", "United Kingdom", "France", "Canada")) %>% 
  mutate(life_exp_rank = rank(lifeExp), .by = year) %>% 
  ggplot() + 
  aes(x = year, 
      y = life_exp_rank, 
      group = country) + 
...
compute_panel_rank <- function(data, scales, flip = F){
  
  data %>% 
  group_by(x) %>% 
  mutate(y = rank(rank_var*!flip))
  
}
  
gapminder::gapminder %>% 
  filter(country %in% 
           c("United States", "Germany", "United Kingdom", 
             "France", "Canada")) %>% 
  ggplot() + 
  aes(x = year, rank_var = lifeExp, group = country) + 
  # geom_ranktrack
  geom_line(stat = statexpress::qstat_panel(compute_panel_rank), flip = F) + 
  geom_point(stat = statexpress::qstat_panel(compute_panel_rank), flip = F) +
  aes(color = country)
...
# but what if you have a lot to say ('need' for layers all over the place)
...
- I have a baby, and 18 cadets, and an data analytics gig
